...
(function(){
  ...
  function install(){
    ...
    function drawHelmet(x,y,s,iron,lean,face){ s=s||1; lean=lean||0; ctx.save(); ctx.translate(x,y); ctx.rotate(lean); var col=iron?"#9aa1a8":"#7a5a34", edge=iron?"#c9ced3":"#9a6c3d"; ctx.fillStyle=col; ctx.beginPath(); ctx.arc(0,0,(face==="side"?8.4:9.4)*s,Math.PI,0); ctx.closePath(); ctx.fill(); ctx.fillStyle=edge; ctx.fillRect(-(face==="side"?9:10)*s,-1*s,(face==="side"?18:20)*s,2.4*s); if(iron){ fillRound(-1.3*s,-9*s,2.6*s,6*s,1,"#dfe3e7"); dot(0,-2*s,1.4*s,"rgba(255,255,255,.5)"); } else { line(-(face==="side"?7:8)*s,-1*s,(face==="side"?7:8)*s,-1*s,1*s,"rgba(60,40,20,.5)"); } ctx.restore(); }
    ...
    function drawThaiHuman(e,sx,sy,opt){
      ...
      var ironHead=/iron|steel|เหล็ก/.test(headId), hasHead=!!opt.head;
